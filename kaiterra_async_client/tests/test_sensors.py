...
class TestGetSensorData(aiounittest.AsyncTestCase):
    ...
    async def test_validate_sensor_ids(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            self.assertTrue(
                client._is_valid_sensor_id(
                    "/lasereggs/00000000-0001-0001-0000-00007e57c0de"
                )
            )
            self.assertTrue(
                client._is_valid_sensor_id(
                    "/laseregg/00000000-0001-0001-0000-00007e57c0de"
                )
            )
            self.assertTrue(
                client._is_valid_sensor_id(
            ...
            self.assertTrue(
                client._is_valid_sensor_id("lasereggs/0000000000010001000000007e57c0de")
            )

            self.assertTrue(
                client._is_valid_sensor_id(
                    "/sensedges/0000000000010001000000007e57c0de"
                )
            )
            self.assertTrue(
                client._is_valid_sensor_id("/sensedge/0000000000010001000000007e57c0de")
            )
            self.assertTrue(
                client._is_valid_sensor_id("sensedges/0000000000010001000000007e57c0de")
            )
            self.assertTrue(
                client._is_valid_sensor_id("sensedge/0000000000010001000000007e57c0de")
            )
            self.assertTrue(
                client._is_valid_sensor_id(
                    "sensedge/00000000-0001-0001-0000-00007e57c0de"
                )
            )

            self.assertFalse(client._is_valid_sensor_id("thing"))
            self.assertFalse(
                client._is_valid_sensor_id("/thing/0000000000010001000000007e57c0de")
            )
            self.assertFalse(
                client._is_valid_sensor_id(
            ...
            self.assertFalse(
                client._is_valid_sensor_id(
                    "/lasereggs/g0000000-0001-0001-0000-00007e57c0de"
                )
            )
            self.assertFalse(
                client._is_valid_sensor_id(
                    "/lasereggs/00000000-0001-00-01-0000-00007e57c0de"
                )
            )
                ...
                [
                    {
                        "rpm10c": {
                            ...
                            "points": [
                                {
                                    "ts": datetime(
                                        2019, 7, 3, 9, 48, 33, tzinfo=timezone.utc
                                    ),
                                    "value": 120.0,
                                    "aqi": 83,
                                }
